...
eflc_logo = lab.get_competition_logo('EFLC') 
efl1_logo = lab.get_competition_logo('EFL1') 
efl2_logo = lab.get_competition_logo('EFL2') 
spl_logo = lab.get_competition_logo('SPL', logo_brighten=True) 
...
specific_label = ["Tyler Morton", "Dylan Levitt", "Leighton Clarkson"]
specific_label_c = 'r'
...
ax.axis("off")
ax.imshow(eflc_logo)
ax = fig.add_axes([0.07, 0.925, 0.05, 0.05])
ax.axis("off")
ax.imshow(efl1_logo)
ax = fig.add_axes([0.018, 0.87, 0.05, 0.05])
ax.axis("off")
ax.imshow(efl2_logo)
ax = fig.add_axes([0.07, 0.87, 0.06, 0.06])
ax.axis("off")
ax.imshow(spl_logo)
...
# Save image
fig.savefig(f"../05_competition_reports_top_players_opta/player_threat_creators/efl_spl-{year}-top_threat_creator_by_zone-{file_ext}-{run_date.replace('/','_')}", dpi=300)
...
# Title
fig.text(0.145, 0.935, title_text, fontweight="bold", fontsize=16, color='w')
fig.text(0.145, 0.905, subtitle_text, fontweight="regular", fontsize=13, color='w')
...
# Save image
fig.savefig(f"../05_competition_reports_top_players_opta/player_threat_creators/efl_spl-{year}-diamond-{run_date.replace('/','_')}-{left_metric}-vs-{right_metric}-player-variant", dpi=300)
