...
playerinfo_df = sde.group_player_events(passes, playerinfo_df, event_types=['pre_assist', 'pass_into_box', 'progressive_pass', 'under_pressure', 'pass_goal_assist'], primary_event_name='passes')
playerinfo_df = sde.group_player_events(passes, playerinfo_df, group_type = 'sum', event_types=['xg_assisted'])
playerinfo_df = sde.group_player_events(successful_passes, playerinfo_df, event_types=['under_pressure'], primary_event_name='successful_passes')
...
# Interceptions
interceptions = midfielder_events[(midfielder_events['type']=='Interception') & (midfielder_events['interception_outcome']!='Lost')]
playerinfo_df = sde.group_player_events(interceptions, playerinfo_df, primary_event_name='interceptions')

# Recoveries
recoveries = midfielder_events[(midfielder_events['type']=='Ball Recovery') & (midfielder_events['ball_recovery_recovery_failure']!=True)]
...
for normalised_col in ['pressures', 'tackles', 'interceptions', 'recoveries']:
    playerinfo_df[normalised_col + '_100pass'] = round(100*playerinfo_df[normalised_col]/playerinfo_df['opp_pass'], 2)
...
for player_choice in players_considered:
    ...
    player_def_hull = sce.create_convex_hull(defensive_actions_df[defensive_actions_df['player_nickname']==player_choice], name=player_choice, include_percent = 75)
    player_off_hull = sce.create_convex_hull(offensive_actions_df[offensive_actions_df['player_nickname']==player_choice], name=player_choice, include_percent = 75)
...
for _ , name in sorted_df.head(12).iterrows():
    ...
    hist = ax['pitch'][idx].imshow(100*hist_data[0]/name['pressures'], extent=[0,120,0,80],cmap=plt.cm.viridis, vmin=0, vmax=6)
    ax['pitch'][idx].text(3,71, "Total:", fontsize=8, fontweight='bold', color='w', zorder=3)
    ...
    ax['pitch'][idx].text(49,77, f"{round(name['pressures_100pass'],1)}", fontsize=8, fontweight ='bold', color='w', zorder=3)
    idx+=1
...
fig.text(0.11,0.93, title_text, fontweight="bold", fontsize=16, color='w')
htext.fig_text(0.11,0.915,s=subtitle_text,fontweight="regular",fontsize=14,color='w',
         highlight_textprops = [{"color": 'cornflowerblue',"fontweight": 'bold'},{"color": 'mediumseagreen',"fontweight": 'bold'}]) 
