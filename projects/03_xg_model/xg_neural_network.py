...
neural_net.add(tf.keras.layers.Flatten())
neural_net.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))
neural_net.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))
neural_net.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))
neural_net.add(tf.keras.layers.Dense(2, activation = tf.nn.softmax))
...
# Train model
neural_net.fit(X, y, epochs=5)						

# %% Make prediction

#  xG
shots_model_df['xG'] = neural_net.predict([X])[:,1]

# %% Save xG data

with bz2.BZ2File("../../data_directory/misc_data/neural_net_xg_data.pbz2", "wb") as f:
